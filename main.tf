...
resource "aws_instance" "shibidp" {
  ...
  provisioner "local-exec" {
    command = "sleep 30; ERR=255; while [ $ERR -gt 0 ]; do ssh -oConnectTimeout=1 -oPasswordAuthentication=no -oUserKnownHostsFile=/dev/null -oStrictHostKeyChecking=no centos@${aws_instance.shibidp.public_ip} exit; ERR=$?; sleep 5; done; knife bootstrap -y ${aws_instance.shibidp.public_ip} -N ${var.aws_r53_record_name} -x centos --sudo $( [ \"${var.chef_runlist}\" != \"\" ] && echo -r '${var.chef_runlist}')"
  }
